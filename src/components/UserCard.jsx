...
const UserCard = ({ user }) => {
  ...
  const isOnline = onlineUsers.some(onlineId => String(onlineId) === String(_id));
      ...
      <div className="card-body">
        ...
        <div className="flex flex-wrap gap-2 my-2">
          {skillsArray && skillsArray.map((skill, index) => (
            <div key={index} className="badge badge-outline badge-sm uppercase font-mono text-xs">
              {skill}
            </div>
          ))}
        </div>
        ...
        {!user?.user && _id && (
          <div className="card-actions justify-center mt-4 gap-4">
            <button className="btn btn-primary px-8" onClick={() => handleSendRequest("ignored", _id)}>
              Ignore
            </button>
            <button className="btn btn-secondary px-8" onClick={() => handleSendRequest("interested", _id)}>
              Interested
            </button>
          </div>
        )}
